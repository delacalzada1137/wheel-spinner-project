...
function addStudent() {
  ...
  if (newName) {
    ...
    var checkbox = $(document.createElement('input')).attr({
      ...
      var segments = wheel.segments;
      var i = segments.indexOf(newName);
      if (this.checked && i == -1) {
...
var wheel = {
  ...
  spin: function () {
    ...
    if (wheel.timerHandle == 0) {
      ...
      wheel.maxSpeed = Math.PI / (1 + (Math.random() * 5)); 
      wheel.frames = 0;
  ...
  onTimerTick: function () {
    ...
    while (wheel.angleCurrent >= Math.PI * 2)
        ...
        if (finished) {
          ...
          wheel.timerHandle = 0;
          wheel.angleDelta = 0;
          var winningIndex = wheel.segments.length - Math.floor((wheel.angleCurrent / (Math.PI * 2)) * wheel.segments.length) - 1;
          var winnerName = wheel.segments[winningIndex];
          console.log('Winner: ' + winnerName);
          winners.push({ name: winnerName });
          $('#student-' + winnerName).prop('checked', false);
          wheel.segments.splice(winningIndex, 1);
          congratulateWinner(winnerName);
          alert('Congratulations, ' + winnerName + '!');
          function congratulateWinner(winnerName) {
            console.log('Congratulations, ' + winnerName + '!');
            $('#winners ul').append('<li>' + winnerName + '</li>');
          }
